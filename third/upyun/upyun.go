...
func (u *UpYun) httpAction(method, uri string, headers map[string]string,
	...
	if method == "PUT" || method == "POST" {
		method = "POST"
		if inFile != nil {
			md5, err := IoMd5(inFile)
			if err != nil {
				if u.Debug {
					panic("IoMd5 failed: " + err.Error())
				}
				return "", err
			}
			...
			if err != nil {
				if u.Debug {
					panic("inFile.Seek failed: " + err.Error())
				}
				return "", err
			}
	...
	if u.Debug {
		...
		if err != nil {
			panic("httputil.DumpRequestOut failed: " + err.Error())
		}
	...
	if err != nil {
		if u.Debug {
			panic("httpClient.Do failed: " + err.Error())
		}
		return "", err
	}
